...
for ref_genome in edited["reference_genome"]:
        ...
        if config[ref_genome]['species'] not in config:
                config[config[ref_genome]['species']] = {}
                config[config[ref_genome]['species']]['star_index'] = st.number_input("number for STAR genomeSAindexNbases", min_value=12, max_value=16, value=12)
                config[config[ref_genome]['species']]['genomesize'] = st.number_input("genome size:", value=1.3e8, format="%.2e")
                config[config[ref_genome]['species']]['ncbiID'] = st.text_input("NCBI species ID", value="3702") 
                config[config[ref_genome]['species']]['genus'] = st.text_input("genus", value="Arabidopsis")
                config[config[ref_genome]['species']]['go_database'] = st.text_input("GO database", value="org.Athaliana.eg.db", help="pattern should be org.<firstlettergenus><species>.eg.db")
